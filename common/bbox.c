...
{
	return (BBox3) {
		.min = double3_min(min, max),
		.max = double3_max(min, max)
	};
}
...
{
	return bbox_create(
		(double3) double3_min(b.min, p),
		(double3) double3_max(b.max, p)
	);
}
...
{
	return bbox_create(
		(double3) double3_min(b1.min, b2.min),
		(double3) double3_max(b1.max, b2.max)
	);
}
...
{
	return bbox_create(
		(double3) double3_max(b1.min, b2.min),
		(double3) double3_min(b1.max, b2.max)
	);
}
...
BBox3 bbox_p_expand(BBox3 b, double delta)
{
	...
	return bbox_create(
		double3_diff(b.min, v_delta),
		double3_add (b.max, v_delta)
	);
}

double3 bbox_diagonal(BBox3 b)
{
	return double3_diff(b.max, b.min);
}

double bbox_surface_area(BBox3 b)
{
	double3 diag = bbox_diagonal(b);
	return 2 * double3_norm_2(diag);
}
...
{
	return double3_lerp(b.min, b.max, p);
}
